...
// service to get pose
// Include the headers for the custom messages and services
#include <social_navigation_layers/GetApproachingPose.h>
#include <social_navigation_layers/RobotPosition.h>
#include <social_navigation_layers/RecognitionState.h>
// circle fitting
...
ros::Subscriber recog_state_sub;
...
bool getApproachingPose(costmap_display::GetApproachingPose::Request &req,costmap_display::GetApproachingPose::Response &res)
{
...
visualization_msgs::Marker createMarkerHumanBody(float_t x, float_t y, float_t z, int id_point, int id_zone){
    ...
    if ((recognized_visualze_flag == true) && (isCorrRecog == false))// start recognition
    {
    ...
    else if ((recognized_visualze_flag == false) && (isCorrRecog == true)) // recognize correctly => green
    {
...
std::vector <geometry_msgs::Pose2D> humanGroupDetection(gazebo_msgs::ModelStates human_states){
    ROS_INFO("Detecting human_group");
    ...
    {
        if((human_states.name[i] == "actor1")||(human_states.name[i] == "actor2"))
        {
    ...
    return vpose;
    ROS_INFO("Finish Detect human_group");
}
//II.4====================================================================================//
...
//V=====================================================================================//
void apprPoseCommandCallback(costmap_display::RecognitionState msg)
{
    appr_visualize_flag = msg.isCorrect;
}
//VI====================================================================================//
void endApprPoseCommandCallback(costmap_display::RecognitionState msg)
{
    appr_visualize_flag = msg.isCorrect;
    recognized_visualze_flag = msg.startRecog;
    isCorrRecog = msg.isCorrect;
}
//VII===================================================================================//
void recogStateCallback(costmap_display::RecognitionState msg)
{
    recognized_visualze_flag = msg.startRecog;
    isCorrRecog = msg.isCorrect;
}
//VIII==================================================================================//
void goalIndexCallback(costmap_display::RobotPosition msg)
{
...
gazebo_msgs::ModelStates getHumanPoseFromGazebo(gazebo_msgs::ModelStates msg){
    ...
    for(int i =0; i<msg.name.size(); i++){
        if((msg.name[i] == "actor1")||(msg.name[i] == "actor2")||(msg.name[i] == "actor3")||(msg.name[i] == "actor4")||(msg.name[i] == "actor5")
           ||(msg.name[i] == "aactor6")||(msg.name[i] == "rp_denis")||(msg.name[i] == "rp_denis_0")||(msg.name[i] == "female")||(msg.name[i] == "rp_mei")
...
//   approach_pose_command_sub = nh.subscribe("/detect_and_recog/recog_state_command", 10, apprPoseCommandCallback);
...
//   recog_state_sub = nh.subscribe("/detect_and_recog/recog_state_command", 10, recogStateCallback);
