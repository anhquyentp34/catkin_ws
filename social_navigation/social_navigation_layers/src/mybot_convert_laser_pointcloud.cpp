...
gazebo_msgs::ModelStates My_Filter::humanDetectionFromGazebo(geometry_msgs::Pose2D cur_rpos, sensor_msgs::PointCloud cloud_laser,gazebo_msgs::ModelStates msg)
{
    ...
    // double max_dis = 8.0; //Thầy
    double max_dis = 6.0;  // Tiến edited
    double max_dxy = 0.4;
    ...
    for(int i=0; i<msg.name.size(); i++)
    {
        ...
        double ang = atan2(dy,dx);
         ...
         if(d<max_dis)
         {// maximum distance of laser data
            double diff = angles::shortest_angular_distance(cur_rpos.theta, ang);
            bool flag = false;

            // if(d<1.8)  // Close by Tien
            // {
            //     flag = true;
            // }else{
                if(fabs(diff)<0.785){ // 0.75 = 45/180*3.14 xet laser 90 do; 2.09 = 120/180*3.14 -- 240^o - for original
                    //bool flag = false;
