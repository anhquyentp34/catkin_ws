...
//                                        {-8.864045540438656, 2.047309735095137, -0.0070359868513256586, 0.9999752471381619}};
 double_t landmarks_pose[N_GOALS][4] = {{0.945546233456496, 4.844177606842447, 0.008298503988575642, 0.999965566822954}, {2.4, 5.49, 0.2774615212998381, 0.9607367507272632}, {6.39565, 2.158565263387091, 0.043200878538880126, 0.9990663829498043},
                                        {5.280866581849011,-1.1399355679074732, -0.6918911709326544, 0.7220018058048334}, {6.972651073014089, -4.603245977323878, -0.5566936207707035, 0.8307178898959647}, {3.8429206531658955, -7.746525935356161, -0.9742618110405334, 0.22541943915292642},
                                        {-1.7296581091340602,-7.6707138250513385, 0.9930704503662334, 0.1175205539869785}, { -7.717271921838065, -8.000284112283271, 0.9997861699247933, 0.020678840081402763}, {-4.5178988226407775, -3.620218026081892,  0.993167700436783, -0.11669563636773454},
                                        {-8.864045540438656, 2.047309735095137, -0.0070359868513256586, 0.9999752471381619}};
...
void approaPoseCommandCallback(social_navigation_layers::RecognitionState msg)
{
    bApproaching.data = msg.isCorrect;
    //ROS_INFO("b", msg);
   // return 0;
}
int main(int argc, char** argv)
{
  ...
  // goal_index_pub = nh.advertise<std_msgs::Int8>("/send_goal_pose/goal_index",100);
   ...
   //end_approach_pose_command_pub = nh.adrovertise<std_msgs::Bool>("/send_goal_pose/end_approach_pose",10);
   end_approach_pose_command_pub = nh.advertise<social_navigation_layers::RecognitionState>("/send_goal_pose/end_approach_pose",10);
   ...
   approach_pose_command_sub = nh.subscribe("/detect_and_recog/recog_state_command", 10, approaPoseCommandCallback);
        ...
        if(app_pose_client.call(srv))
        {
               ...
               (update_goal_pose.position.y!=update_goal_pose_old.position.y))
            {
               ...
               if(action_goal.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
               {
                 ...
                 // social_navigation_layers::RecognitionState end_approach_pose_msg;
                 // end_approach_pose_msg.startRecog = false;
                 // end_approach_pose_msg.isCorrect = false;
